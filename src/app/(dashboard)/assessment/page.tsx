      ...
      <div className="flex items-start gap-4 mb-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-xl font-bold text-gray-900">Assessment Generator</h1>
          ...
          <p className="text-gray-500 text-sm">
            Generate 10 objective questions and 3 theory questions with answer keys.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              WAEC/NECO style, Bloom&apos;s taxonomy, 50+ questions and marking scheme.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          {loading ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-12 flex flex-col items-center justify-center gap-4">
              ...
              <p className="font-semibold text-gray-900">Generating assessment...</p>
            </div>
          ) : result ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center border-dashed">
                <p className="font-bold text-gray-900 mb-1">SCHOOL NAME: _________________________</p>
                <p className="text-xs text-gray-500">
                  Class: <strong>{result.classLevel}</strong> &nbsp;|&nbsp;
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden">
                ...
                <div className="p-4 space-y-4 max-h-64 overflow-y-auto">
                  {result.objectiveQuestions?.map((q: any, i: number) => (
                    <div key={i} className="pb-3 border-b border-gray-50 last:border-0">
                      ...
                      <div className="grid grid-cols-2 gap-1">
                        {q.options?.map((opt: string, j: number) => (
                          <p key={j} className={`text-xs px-2 py-1 rounded ${opt === q.answer ? "text-teal-700 font-semibold bg-teal-50" : "text-gray-500"}`}>
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  {result.theoryQuestions?.map((q: any, i: number) => (
                    <div key={i} className="flex gap-3">
                      <span className="text-sm font-bold text-teal-600 flex-shrink-0">Q{i + 1}.</span>
                      <div>
                        <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-line">{q.question}</p>
              ...
              <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl flex items-center gap-2">
                ...
                <p className="text-xs text-amber-700 font-medium">
                  Pro: WAEC/NECO style, Bloom&apos;s taxonomy, 50+ questions, marking scheme and school branding.
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-12 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-gray-400 text-sm max-w-xs">Configure your options and click Generate to create your assessment.</p>
            </div>
          )}
        </div>
      </div>
